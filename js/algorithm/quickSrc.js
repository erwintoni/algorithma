/*
	Name:		Mark Mata, Roddy Nguyen, Team (One/Two)
	...
	Functions:
		...
		Initialize() : function that runs as the page is loaded, will pre-run the algorithm and creates the list of naMoves for the algorithm to run
		Res() : function that runs to enable and disable certain buttons in firefox
		Forward() : function that steps forward through the animation one step at a time as the "Forward" button is pushed in the HTML file
		Highlight(variable) : function that highlights a line of psuedocode and a div box
		Undo() : function that undoes all the higlights in the psuedocode
		InitializeTimer() : first of three function required to run the timer for the demo
		StopTheClock() : second of three functions required to run the timer for the demo
		StartTheTimer() : third of three functions required to run the timer for the demo
*/
...
function Initialize()
{
	...
	document.getElementById("howto").onmouseover = function() {
		Tip("<strong>Demo</strong> - Starts the Quick Sort animation.<br /><br /><strong>Forward</strong> - Allows the user to step through the sort one step at a time. Walkthrough is enabled when this button is pressed.<br /><br /><strong>Reset</strong> - Resets the Quick Sort animation to the beginning.",WIDTH, 400, FONTFACE, 'Verdana, sans-serif', FONTSIZE, '12pt',FONTCOLOR, '#000000',PADDING, 8, SHADOW, true, SHADOWCOLOR, '#CCCCCC', FADEIN, 400, FADEOUT, 800,CENTERMOUSE, true, OFFSETX, 0 ,BGCOLOR, '#CEFFFF');
	}
}
