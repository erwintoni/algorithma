/*
	...
	Commented:	2011.03.16
 	...
 	Functions:
		PriorityQueue ( value ) : class takes an initial value (0) that is used to create the priority queue class
		Initialize() : function that runs as the page is loaded, will pre-run the algorithm and creates the list of moves for the algorithm to run
		Swap(nSwp1, nSwp2) : uses jsAnim and pbMan to swap the positions of two different div boxes
		Varcheck(nsVariable) : Checks to see if the variable inputed is "proper"
		...
		Forward() : function that steps forward through the animation one step at a time as the timer goes through the naPqMoves array
