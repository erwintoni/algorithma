/*
	Name:		Sean Siefert, Michael Stevens
	Project:		{AlgorithmA}; 2010
	File:		criticalpathSrc-preWed.css
	Created:		2010.03.10
...
var posx2 = new Array(startx + 40, startx + 40);
var posy2 = new Array(starty - 30, starty + 30);

var parents = new Array([-1], [0], [0, 1], [1, 2], [2, 3], [3, 4]);
var activties = new Array();
var maxParent = new Array(6);
var maxWeights = new Array(6);
...
/*
{parents[i][j] and activityTime[i][j] are parallel, two-dimensional arrays}

procedure findWeights(parents[1…n])

FOR i = 1 to length of parents do

      {iterates through each node}

      max = 0

      FOR j = 0 to length of parents[i] do

            {iterates through parents of node i}

            IF maxWeight[parents[i][j]] + activityTime[i][j] > max

                  {find longest path to current node}

                  max = maxWeight[parents[i][j]] + activityTime[i][j]

                  maxParent[i] = parents[i][j]

      maxWeight[i] = max

procedure buildCriticalPath(maxParent[0…n])

      currentNode = last node;

      WHILE currentNode is not 0

            currentNode = maxParent[currentNode];
...
var drawBlue = new Array(false, false, false, false, false, false);
...
var dRed = new Array(false, false);
var dBlue = new Array(false, false);
...
function Initialize()
{
    ...
    {
        Tip("<strong>Build Times/Play</strong> - This starts the process to trace the calcuation of the cumulative time of each path./><br /><strong>Build CP</strong> - This starts the animation to trace the critical path from the start to finish. <br /><br /><strong>Start Over</strong> - This resets the page to a fresh load.", WIDTH, 400, FONTFACE, 'Verdana, sans-serif', FONTSIZE, '12pt', FONTCOLOR, '#000000', PADDING, 8, SHADOW, true, SHADOWCOLOR, '#CCCCCC', FADEIN, 400, FADEOUT, 800, CENTERMOUSE, true, OFFSETX, 0, BGCOLOR, '#CEFFFF');
    }
    for (i = 0; i < 9; i++) 
    {
...
function buildLines()
{
    ...
    drawArrowLeft(mnodes[3], (dBlue[0] ? "#3798ff" : dRed[0] ? "#d35050" : "#57c34e"));
    ...
    drawArrowLeft(mnodes[5], (dBlue[1] ? "#3798ff" : dRed[1] ? "#d35050" : "#57c34e"));
}
...
function drawPsuedo()
{
    ...
    jgp.clear();
    jgp.setFont("verdana", "12px", Font.standard);
    
    for (i = 0; i < psuedoLines.length; i++) 
    {
        jgp.setColor(psuedoColors[i]);
        jgp.drawString(psuedoLines[i] + "<br>", -240, 66 + i * 30);
    }
    jgp.paint();
}
...

{
    ...
    for (i = 0; i < maxWeights.length; i++) 
    {
        ...
        jg.drawString(maxWeights[i], mnodes[i].offsetLeft + boxWidth, mnodes[i].offsetTop - 10);
    }
}
...

{
    ...
    for (i = 0; i < 6; i++) 
    {
        if (drawBlue[i]) 
        {
            mnodes[i].style.background = blueImg;
        }
    }
    jg.paint();
}
...
function buildActivities()
{
    //Inputs: 	none
    //Outputs: none
    //Purpose:	This function builds various arrays. Each array holds specific information. The main array holding the pertinent data is 				criticalPath.
    //	criticalPath will hold the path to take, by node from the start to finish to show the critical path. 
    
    activities = new Array([0], [parseInt(lineTimes[0])], [parseInt(lineTimes[1]), parseInt(lineTimes[2])], [parseInt(lineTimes[3]), parseInt(lineTimes[4])], [parseInt(lineTimes[5]), parseInt(lineTimes[6])], [parseInt(lineTimes[7]), parseInt(lineTimes[8])]);
    maxParent[0] = 0;
    jgp.setFont("verdana", "12px", Font.standard);
    maxParent[1] = parents[1][0];
    var max = 0;
    maxWeights[0] = 0;
    maxWeights[1] = activities[1][0];
    for (i = 2; i < 6; i++) 
    {
        max = 0;
        ...
        {
            if (maxWeights[parents[i][j]] + activities[i][j] > max) 
            {
                max = maxWeights[parents[i][j]] + activities[i][j];
                maxParent[i] = parents[i][j];
            }
        }
        maxWeights[i] = max;
...

{
    ...
    else if (animating && animW) 
        
        {
            animW = false;
            animateCP(-1, paths[buildWeights()].split(" "));
        }
        else if (animating && animCP) 
            {
                animCP = false;
                animating = false;
            }
...
function animateTimes(indexNodes, indexPsuedo)

{
    //Inputs:	indexNodes - the current index being worked on
    //			indexPsuedo - the current counter of the psuedoCode
    //Outputs:	none
    //Purpose:	To animate the building of the weights for every node
    
    if (indexNodes == -1 && !animW) 
    {
        animW = true;
        animateTimes(indexNodes + 1, 0);
    ...
    if (indexPsuedo % 2) 
    {
        ...
        {
            if ((indexNodes + " " + maxParent[indexNodes]) == pathNames[i]) 
            {
                lineColors[i] = "#d35050";
                i = pathNames.length;
            }
        }
        mnodes[indexNodes].style.background = redImg;
        psuedoColors[1] = "#d35050"
        switch (indexNodes)
        {
            case 3:
                dRed[0] = true;
        ...
        draw();
        window.setTimeout(resetPsuedo, 500, indexNodes, indexPsuedo + 1);
    }
    else 
    {
        dRed = new Array(false, false);
        psuedoColors[2] = "#d35050";
        weightColors[indexNodes] = "#d35050";
        mnodes[indexNodes].style.background = redImg;
        draw();
        window.setTimeout(resetPsuedo, 500, indexNodes + 1, indexPsuedo + 1);
    }
}


function resetPsuedo(indexNodes, indexPsuedo)
{
    //Inputs:	indexNodes - index of the node currently being used
    //			indexPsuedo - counter of the psuedoCde
    //Outputs:	none
    //Purpose:	To control the animationg of the weight building. It resets the appropriate line colors and stops the animation whend one
    
    if (indexNodes < mnodes.length) 
    {
        for (i = 0; i < psuedoColors.length; i++) 
        {
            psuedoColors[i] = "black";
        }
        lineColors = lineColorsStart.slice(0);
        mnodes[indexNodes].style.background = greenImg;
        animateTimes(indexNodes, indexPsuedo);
    }
    else 
    {
        animate();
...
{
    //Inputs:	none
    //Outputs:	The index of the longest path
    //Purpose:	This functions looks at the defined lists in the html file, adds up their weight, and returns the infex of the longest path	
    
    var tempWeights = new Array();
    ...
    {
        tempTotal = 0;
        tempTokens = paths[i].split(" ");
        for (j = 0; j < tempTokens.length; j++) 
        {
            tempTotal += parseInt(lineTimes[parseInt(tempTokens[j])]);
        }
        if (tempTotal > currentLongest) 
        {
            currentLongest = tempTotal;
            longestIndex = i;
        }
    }
    return longestIndex;
...
function animateCP(index, tokens)

{
    //Inputs:	index - the index of the currently working node. Must pass -1 to start the animation
    //			tokens - a tokenized version of the pre-defined paths
    //Outputs:	none
    //Purpose:	To animate tracing the critical path
    
    if (index == -1 && !animCP) 
    {
        reset();
        animCP = true;
        window.setTimeout(animateCP, 1500, index + 1, tokens);
    }
    else 
        if (animCP) 
            if (index < tokens.length) 
            {
                ...
                lineColors[parseInt(tokens[index])] = "#3798ff";
                draw();
                window.setTimeout(animateCP, 1500, index + 1, tokens);
            }
            else 
...
function resetTimes()
{
    ...
    for (i = 0; i < 9; i++) 
    {
        document.getElementById("form" + String.fromCharCode(uniModCode + i) + "data").value = lineTimes[i] + '';
    }
}
...
/*
 ...
 //Inputs: 	none
 //Outputs:	none
 //Purpose:	To start animating the weight building
 ...
 //Inputs: 	none
 //Outputs:	none
 //Purpose:	To start animating the critical path
