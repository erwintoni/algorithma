/*
	...
	Description:		
		...
		This library is made up of three layers.
			* Top Level - Manager
				- Made up of jsAnim's Manager class. 
				- Holds a list animation objects which are HTML DOM objects which will
					have animations appended to them.
				- This manager continuously calls itself using "step()" every 40ms (timestep) 
					by default
				- Each step is a single frame. Each frame is timed out every 40ms (timestep)
				- The step runs every object held within its animation object *MODIFIED*
				- You create animation objects here with "createAnimObject"
			* Middle Level - Animation Object
				- Maintained by the Manager class.
				- Holds a dynamic list of animation entries that only runs one animation at 
					a time with "step"
				- Keeps track of which animation it is on. Can eventually be reset back to
					the beginning to replay all animations held within.
				- You add animation parameters here with "add"
		* Low Level - Animation Entry
			- Maintained by the Animation Object
				- Holds a single animation entry with all the properties to run it.
				- Makes use of helper classes and container classes to create and run the 
					specified animation
				- Step here only does one frame at a time, but this actually goes out to 
					modify whatever object assigned to it.
				- User does NOT use this layer. Internal Only.
				
	Changelog:
		All sections modified have been specified with a *modified* moniker
		
		March 22, 2010
			* New function added to Prop, Prop.lineRegistered
				- A copy and paste of Prop.line, but now it works with Registered Position.
				- Look at the homepage for an example of this!
			* New function added to jsAnimManager, registerPositionEl
				- Instead of taking in a string for an ID, it takes in an element that has already been found.
				- Used for Prop.lineRegistered

		March 08, 2010
			* New function added to Prop, Prop.line
				- Draws a line by creating and placing div box objects.
				- Takes in Attrib, a new helper function class as input.
				- Can change position, size, and color.
				- Requires the parent object as when creating a line animation.
				- Does NOT use register position.
				- Global arrays hold a history and list of all div boxes created*				- Supports Easing.
			* New helper functions: Attrib, EraseLine
				- Attrib is a class function that is a combination of Col, Pos, Dim classes.
					* Only Prop.line uses this.
			- EraseLine is a function that erases a specified line within a particular parent object.
		* New functions added to jsAnimObject: "runall" & "runone"
				- Sets a flag on the particular animation object so as to run all properies at once
				rather than one at a time.
			- Do not abuse this!
		February 26, 2010
			* Fixed registeredposition to do what it was originally intended to do.
	February 14, 2010
		* New function added to jsAnimObject "sequential" & "nonsequential" to Animation 
				Object
			- Sets a flag per object that this must be done before any other
				animation is done below it.
		* New global flag "sequentID"
			- Keeps track of the first instance a squential block of animations
				- Used in jsAnimManager's step function to do the above functionality
				- Used in pbMan to make sure an animation is not being run sequentially
...
// This is used to hold whether or not sequential is running.
var sequentID = -1;

// This array holds a matrix array of lines and the divs that make them up.
...
function jsAnimManager(timestep) 
{
	...
	/* Modified */
		// Does all animations until it hits a sequential flag.
		// Will not continue until object with sequential flag is finished.
	this.step = function() {
		if(!this.paused) {
			this.complete = true;
			for(var x in this.animObjects) {
				if (this.animObjects[x].isSequent && sequentID == -1)
						sequentID = x;
				if (sequentID == -1 || x <= sequentID)
					this.animObjects[x].step();
				if (x == sequentID && this.animObjects[x].complete == true)
					sequentID = -1;
				if (!this.animObjects[x].complete)
					this.complete = false;
			}
	...
	*/
	...
	Called to set the appropriate style values to allow position to be controlled by jsAnim
		- Uses the center of each object, not the top left corner.
	Params:
		-objId : id of object to be registered
		-[fixed] : fixed positioning, false to absolute fixed, defaults to false
	*/
	// WANRNING!
	// Below sets up your object within its container with the top center being (0,0).
	// This is dangerous. You must keep in mind the change in coordinates.
	/* Modified */
	...
	/* MODIFIED */
	// Same as above only it takes in an element, not an ID.
	this.registerPositionEl = function(el, fixed) {
		var width = el.offsetWidth;
		var height = el.offsetHeight;
...
function jsAnimObject(obj, id, manager) {
	...
	*/
	...
	/* MODIFIED */
		if (property == Prop.line || property == Prop.lineRegistered)
		{
			lineIndex++;
			var line = lineIndex;
			lineHistory[line] = new Array();
		}
		else
			var line = -1;
						
		this.animEntries[this.current] = new jsAnimEntry(this.obj, property, from, to, duration, this.manager.timestep, ease, onLoop, onComplete, line);
...
/* Modified */
function Attrib(x, y, radius, red, green, blue)
{
	this.radius = radius;
	this.x = x;
	this.y = y;
...
/* MODIFIED */
/* Instead of relying on Register Position, use this for Prop.position */
function setPosition2 (el, x, y)
{
	el.style.left = x + "px";
	el.style.top = y + "px";
}
...
/* MODIFIED */
// Erases a line. Must pass the jsAnimManager, parent id string, and the line index id.
function eraseLine(manager, parent, ID)
{
	if (manager.complete)
	{
		...
		var length = lineHistory[ID].length;
		for(var i = 0; i < length; i++)
		{ // Perm removal of the object
			parentObj.removeChild(document.getElementById(lineHistory[ID].pop()));
		}
	}
}
...
/* MODIFIED */
function jsAnimEntry(obj, property, from, to, duration, timestep, ease, onLoop, onComplete, line) {
	...
	*/
	...
	this.step = function() {
		...
		if(this.current >= this.duration) {
			...
			if(this.property != Prop.line && this.property != Prop.lineRegistered)
				this.property.update(this.obj, this.from, this.to, p);
			else 
				this.property.update(this.obj, this.from, this.to, p, this.lineID);
			return true;
		}
		else {
...
var Prop = {
	...
	*/
	...
	/* MODIFIED 
		Draws a line starting from a position, size, and color to another position,
		size, and color. The line must have a radius. Can set the box to transparent 
		by setting colors to a negative number. This is the only Prop to use 5 params
		for update, since it needs an ID for the line (jsAnim works per frame with 
		everything at once, doesn't work otherwise).
		
		current is not used, its used when "from" is not specified.
	*/
	
	line : {
		update : function(obj, from, to, percent, lineID) {
			var x = jsAnimUtil.interp(from.x, to.x, percent);
			var y = jsAnimUtil.interp(from.y, to.y, percent);
			if (to.radius)
				var radius = jsAnimUtil.interp(from.radius, to.radius, percent);
			else
				var radius = 5;
				
			var lineSize = lineHistory[lineID].length;
			var red = jsAnimUtil.interp(from.red, to.red, percent);
			var green = jsAnimUtil.interp(from.green, to.green, percent);
			var blue = jsAnimUtil.interp(from.blue, to.blue, percent);
				
			var id = 'line'+lineID+'-'+lineSize;
			...
			setPosition2(element, x, y);
			
			lineHistory[lineID].push(id);
	...
	/* MODIFIED
		This is for registered positions only. Works great when you want everything 
		to be centered at all times. See the homepage for an example!
	*/
	lineRegistered : {
		update : function(obj, from, to, percent, lineID) {
			var x = jsAnimUtil.interp(from.x, to.x, percent);
			...
			manager.registerPositionEl(element);
	...
	*/
	/* Modified */
	// NOTE - Now works with both registered positions and normal positions
	position : {
		update : function(obj, from, to, percent) {
			var x = jsAnimUtil.interp(from.x, to.x, percent);
			var y = jsAnimUtil.interp(from.y, to.y, percent);
		...
		current : function(obj) {
			...
			{
				var left = parseInt(jsAnimUtil.getCSS(obj, 'left'));
				var top = parseInt(jsAnimUtil.getCSS(obj, 'top'));
				var x = left;
				var y = top;
				return new Pos(x,y);
			}
	...
	*/
	// NOTE - Only works with Registered Positions!
	...
	*/
	...
	color : {
		update : function(obj, from, to, percent)  {
			var r = jsAnimUtil.interp(from.r, to.r, percent);
			var g = jsAnimUtil.interp(from.g, to.g, percent);
			var b = jsAnimUtil.interp(from.b, to.b, percent);
	...
	opacity : {
		update : function(obj, from, to, percent)  {
			var v = jsAnimUtil.interp(100*from, 100*to, percent);			
